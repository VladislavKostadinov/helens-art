...
<div id="wrapper" class="container content-section text-center">
...
exit;
         ...
         $sql = "SELECT art.* FROM art WHERE art.id = '$id'";
...
<!-- [THE IMAGES] --><div class="mainPic">
<!-- <h1 class="pic-title fHeadings" style="color:white;margin-top:27px;padding-top:35px;"><?php echo $title; ?></h1> -->

<img class="zoomD" src="<?php echo IMG . $img; ?>" />
<div class="book-main">
                  </div></div>
	
    </div> </div>
                <div id="similiarProjects">
    <h4 class="simi-p fHeadings" style="display:table; margin: auto; 
border-bottom: 2px solid burlywood; border-top: 2px solid burlywood; padding: 7px 0;">
Подобни издания</h4>

<?php 
         $sql_sim = "SELECT DISTINCT art.* FROM art WHERE art.type = $type  AND  art.id != $id ORDER BY RAND() LIMIT 4";

         $result_sim = mysqli_query($conn, $sql_sim);

if (mysqli_num_rows($result_sim) > 0) {
...
<a href="<?php echo URLBASE ?>/view-art.php?id=<?php echo $row['id']; ?>&type=<?php echo $row['type']; ?>#single-project">
 <img src="<?php echo IMG . $img_src; ?>" style="width:166px;height:249px;" class="img-responsive sim" />
</a>
 </div>
<?php }} ?>
<p style="text-align:center;">
				<a href="/contact.php#contact" class="btnghost"><i class="fa fa-envelope"></i>Направи поръчка</a> 
				<i id='or' style="font-size:12px;margin-right:3px;"> или </i> 
				<a href="/index.php#hotspot" class="btnghost"><i class="fa fa-camera"></i>Върни се на галерия</a>
			</p>
</div></div>
</section></div>
<!-- Footer -->
